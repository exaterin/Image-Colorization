...
class ModelInception(nn.Module):
    def __init__(self):
        ...
        # Encoding layers
        self.enc1 = conv_block(1, 64)    # 256x256x1 -> 128x128x64
        ...
        # Additional layer to match feature vector dimensions
        self.fc = nn.Linear(2048, 1024 * 4 * 4)

        # Decoding layers
        self.dec1 = deconv_block(2048, 1024) # 4x4x2048 -> 8x8x1024
    ...
    def forward(self, x, feature_vector):
        ...
        # Reshape feature vector and concatenate
        feature_vector = self.fc(feature_vector).view(-1, 1024, 4, 4)
        e6 = torch.cat((e6, feature_vector), dim=1)
